    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-4 py-2 border-b flex flex-row items-center justify-between">
          <DialogTitle className="truncate text-sm font-medium">Edit {relativePath}</DialogTitle>
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          {isLoading ? (
            ...
            <CodeEditor
              ...
              options={{ wordWrap: 'on', lineNumbers: 'on', minimap: { enabled: false } }}
            />
          )}
        </div>
        ...
        <DialogFooter className="px-4 py-2 border-t bg-muted/50">
          ...
          <Button size="sm" onClick={handleSave} disabled={writeMarkdown.isPending}>
